...
class WindowsInstaller(QObject):
    ...
    def install_windows(self):
        ...
        program_name = "Cloudflare WARP"

        try:
            # 프로그램 이름으로 레지스트리 항목 찾기
            with winreg.OpenKey(winreg.HKEY_LOCAL_MACHINE, r"SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall", 0,
                                winreg.KEY_READ | winreg.KEY_WOW64_64KEY) as key:
                for i in range(winreg.QueryInfoKey(key)[0]):
                    subkey_name = winreg.EnumKey(key, i)
                    with winreg.OpenKey(key, subkey_name) as subkey:
                        # DisplayName 키를 이용하여 프로그램 이름 확인
                        display_name = winreg.QueryValueEx(subkey, 'DisplayName')[0]
                        if display_name == program_name:
                            # UninstallString 키를 이용하여 언인스톨러 실행
                            uninstall_string = winreg.QueryValueEx(subkey, 'UninstallString')[0]
                            subprocess.run(uninstall_string, check=True)
                            print(f"{program_name}을(를) 제거했습니다.")
                            break
        except:
            pass
